...
import {
  ...
  AlertTriangle, ChevronUp, Flag,
} from 'lucide-react'

const PRIORIDADE: Record<string, { label: string; cor: string; dot: string }> = {
  baixa:   { label: 'Baixa',   cor: 'text-lead-400',   dot: 'bg-lead-300'   },
  normal:  { label: 'Normal',  cor: 'text-blue-600',   dot: 'bg-blue-400'   },
  alta:    { label: 'Alta',    cor: 'text-amber-600',  dot: 'bg-amber-400'  },
  urgente: { label: 'Urgente', cor: 'text-red-600',    dot: 'bg-red-500'    },
}
...
export default function TarefasPage() {
  ...
  const [obra, setObra]         = useState<{ nome: string; codigo: string } | null>(null)
  ...
  const [nova, setNova] = useState({
    titulo: '', responsavel: '', prioridade: 'normal', data_prevista: '',
  })
  ...
  async function carregar() {
    ...
    const [{ data: o }, { data: t }] = await Promise.all([
      ...
      supabase.from('tarefas').select('*').eq('obra_id', obraId).order('concluida').order('prioridade').order('criado_em'),
    ])
    setObra(o)
    setTarefas(t || [])
    setCarregando(false)
  ...
  async function salvarNova(e: React.FormEvent) {
    ...
    if (data) {
      ...
      setNova({ titulo: '', responsavel: '', prioridade: 'normal', data_prevista: '' })
  ...
  return (
    <>
      <Header
        titulo="Tarefas"
        subtitulo={obra ? `${obra.codigo} · ${obra.nome}` : 'Carregando...'}
      />

      <div className="page-body max-w-3xl">

        <Link href={`/dashboard/obras/${obraId}`}
            ...
            <form onSubmit={salvarNova} className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <select
                  ...
                  onChange={e => setNova(p => ({ ...p, prioridade: e.target.value }))}
                  className="select"
                >
                  {Object.entries(PRIORIDADE).map(([v, { label }]) => (
                    <option key={v} value={v}>{label}</option>
                  ))}
                </select>
                <input
                  type="date"
                  value={nova.data_prevista}
                  onChange={e => setNova(p => ({ ...p, data_prevista: e.target.value }))}
                  className="input"
                />
              </div>
              <div className="flex gap-2 justify-end">
                <button type="button" onClick={() => setAdicionando(false)} className="btn-secondary btn-sm">
                  Cancelar
                </button>
        ...
        <div className="card overflow-hidden">
          {carregando ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <CheckCircle2 className="w-8 h-8 text-lead-200 mb-3" />
              ...
              {filtro === 'pendentes' && tarefas.length === 0 && (
                  ...
                  <div key={t.id}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1 flex-wrap">
                        {t.responsavel && (
                          <span className="text-xs text-lead-400">{t.responsavel}</span>
                        )}
                        {t.data_prevista && (
                        ...
                        {t.concluida && t.concluida_em && (
                          <span className="text-xs text-emerald-500">
                            Concluída em {fmtData(t.concluida_em.split('T')[0])}
                          </span>
                        )}
                      </div>
                    </div>
