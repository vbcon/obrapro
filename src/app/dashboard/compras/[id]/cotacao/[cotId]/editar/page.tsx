...
export default function EditarCotacaoPage() {
  const { id, cotId } = useParams<{ id: string; cotId: string }>()
  ...
  useEffect(() => {
    createClient().from('cotacoes').select('*').eq('id', cotId).single().then(({ data }) => {
      if (!data) { router.push(`/dashboard/compras/${id}`); return }
      setForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    setSalvando(true)
    const { error } = await createClient().from('cotacoes').update({
  ...
  async function handleDelete() {
    ...
    await createClient().from('cotacoes').delete().eq('id', cotId)
    router.push(`/dashboard/compras/${id}`)
    router.refresh()
  }
  ...
  return (
    <>
      <Header titulo="Editar Proposta" subtitulo={form.fornecedor_nome} />
      <div className="p-6 max-w-3xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="card p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                ...
                <input type="text" required value={form.fornecedor_nome} onChange={e => setField('fornecedor_nome', e.target.value)} className="input" />
              </div>
              <div className="sm:col-span-2">
                <label className="label">Contato</label>
                <input type="text" value={form.fornecedor_contato} onChange={e => setField('fornecedor_contato', e.target.value)} className="input" />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="text" value={form.condicao_pagamento} onChange={e => setField('condicao_pagamento', e.target.value)} className="input" />
              </div>
            </div>
          </div>
            ...
            <div className="space-y-2">
              ...
              {itens.map((item, i) => (
                <div key={i} className="grid grid-cols-12 gap-2 items-center p-3 bg-lead-50 rounded-lg">
                  ...
                  <div className="col-span-2 text-center text-sm text-lead-700">{item.quantidade}</div>
                  <div className="col-span-2">
                    <input type="number" step="0.01" min="0" value={item.valor_unitario} onChange={e => setItem(i, 'valor_unitario', e.target.value)} placeholder="0,00" className="input text-sm py-1.5 text-right" />
                  </div>
                  <div className="col-span-3">
                    <input type="number" step="0.01" min="0" value={item.valor_total} onChange={e => setItem(i, 'valor_total', e.target.value)} placeholder="0,00" className="input text-sm py-1.5 text-right font-semibold" />
                  </div>
                </div>
              ))}
              <div className="flex justify-end pt-2 border-t border-lead-200">
                <div className="text-right">
                  <p className="text-xs text-lead-500">Total</p>
                  <p className="text-2xl font-bold text-lead-900">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(valorTotal)}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="card p-6">
            ...
            <textarea rows={3} value={form.observacoes} onChange={e => setField('observacoes', e.target.value)} className="input resize-none" />
          </div>

          <div className="flex items-center justify-between">
            <button type="button" onClick={handleDelete} className="inline-flex items-center gap-2 text-sm text-red-500 hover:text-red-700"><Trash2 className="w-4 h-4" />Excluir proposta</button>
